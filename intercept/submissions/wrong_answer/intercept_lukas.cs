...
class Numbertree
{
    static List<long> Dijkstra(int s, List<List<Tuple<int, int>>> g, List<long> dist)
    {
        int n = g.Count;
        List<long> cnt = Enumerable.Repeat(0L, n).ToList();
    ...
    public static void Main()
    {
        ...
        var qq = Enumerable.Range(0, n)
            .Where(x => dist[x] + dist2[x] == dist[t] && cnt[x] * cnt2[x] == cnt[t]);
